...
- (void)dealloc {
    ...
    if (!self.viewLoaded) {// tabBar首页设置了一个wk，不走viewDidLoad却会走dealloc，解决移除观察者崩掉的问题
    ...
    if (self.methodNameArray.count != 0) {
...
// 是否允许请求，我们可以在此方法中拦截webView的url做相应的操作
...
- (void)layoutUI {
    ...
    if (self.navigationController.viewControllers.count > 1) {
        ...
        [backButton setImage:[UIImage imageNamed:@"BaseProject_BackArrow"] forState:(UIControlStateNormal)];
...
- (void)setMethodNameArray:(NSArray<NSString *> *)methodNameArray {
    
    _methodNameArray = methodNameArray;
    
    if (_methodNameArray != 0) {
        
        for (NSString *methodName in _methodNameArray) {
...
- (WKWebView *)webView {
    ...
    if (_webView == nil) {
        ...
        _webView.backgroundColor = Default_VC_Background_Color;
...
- (UIProgressView *)progressView {
    ...
    if (_progressView == nil) {
        ...
        _progressView.frame = CGRectMake(0, Navigation_Bar_Height, Screen_Width, 2.0);
        _progressView.transform = CGAffineTransformMakeScale(1, 2);
        [_progressView setProgress:0.1];
        
        _progressView.trackTintColor = Default_VC_Background_Color;
