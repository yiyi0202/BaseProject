...
ProjectMaskView *maskView = [ProjectMaskView showToView:kWindow withCustomSubview:someAlertView subviewLayout:^{
    ...
    [someAlertView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(kWindow.mas_bottom);
        make.right.equalTo(kWindow).offset(-kFittedWidth(80));
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [someAlertView mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(kWindow.mas_bottom).offset(-(kFittedHeight(180)));
        }];
        [someAlertView.superview layoutIfNeeded];
    }];
}];
...
maskView.tapBlock = ^{
    ...
    [ProjectMaskView hideFromView:kWindow withSubviewAnimation:^NSTimeInterval{
